...
const Layout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex bg-[#f5f5f5] text-[#1f2933] max-md:flex-col">
      <aside className="w-[280px] p-[1.25rem] bg-white border-r border-[#e5e7eb] flex flex-col gap-[1.5rem]
                        max-md:w-full max-md:flex-row max-md:items-center max-md:justify-between max-md:flex-wrap max-md:gap-[1rem]">
        {/* Brand */}
        <div className="flex flex-col gap-[0.75rem]">
          <div className="relative inline-flex items-center gap-[0.65rem] w-fit px-[0.85rem] py-[0.55rem] rounded-full whitespace-nowrap
            ...
            <span
              aria-hidden="true"
              className="w-[36px] h-[36px] rounded-[12px] bg-[#2563eb] shadow-[0_6px_16px_rgba(37,99,235,0.3)] inline-block"
              style={{
                WebkitMaskImage:
        ...
        {/* Nav Links */}
        <nav className="flex flex-col gap-[0.5rem] max-md:flex-row max-md:flex-wrap max-md:gap-x-[0.5rem] max-md:gap-y-[0.25rem]">
          {[
            { to: "/", label: "Dashboard" },
              ...
              className={`no-underline px-[0.8rem] py-[0.6rem] rounded-[0.6rem] text-[0.95rem] transition-all
                ${
                  location.pathname === item.to
                    ...
                    : "text-[#4b5563] hover:bg-[#eef2ff] hover:translate-x-[2px]"
                }`}
            >
              {item.label}
            </Link>
        ...
        <button
          ...
          className="mt-auto px-[0.8rem] py-[0.6rem] rounded-[0.6rem] border border-[#fecaca] bg-[#fff1f2] text-[#b91c1c]
                     font-medium cursor-pointer transition-all hover:bg-[#fee2e2] hover:border-[#fca5a5] hover:-translate-y-[1px]
                     max-md:mt-0"
        >
          Logout
        </button>
      ...
      <main className="flex-1 p-[1.5rem] flex flex-col overflow-x-hidden">
        {children}
      </main>
    </div>
  );
